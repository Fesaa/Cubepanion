...
public class AutoVoteCategoryElement extends SettingElement {
  ...
  public AutoVoteCategoryElement(AutoVoteConfig config, AutoVoteConfiguration configuration, AutoVoteCategory category) {
    super(category.id(), null, category.name(), new String[0]);
    ...
    tags.add(configuration.gameName());
    tags.add(category.name());
    for (var option: category.options()) {
      tags.add(option.name());
    }

    setSearchTags(tags.toArray(String[]::new));
  }
