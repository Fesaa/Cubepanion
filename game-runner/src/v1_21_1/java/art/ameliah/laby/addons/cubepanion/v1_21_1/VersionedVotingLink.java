...
public class VersionedVotingLink extends VotingLink {
  ...
  private final Task clickOnMenu = Task.builder(() -> {
    ...
    if (connection == null || Minecraft.getInstance().player == null) {
      LOGGER.info(getClass(), "Connection or player was null");
      return;
    }
    ...
    float xRot = Minecraft.getInstance().player.getXRot();
    float yRot = Minecraft.getInstance().player.getYRot();
    connection.send(new ServerboundUseItemPacket(InteractionHand.MAIN_HAND, sequence, xRot, yRot));
